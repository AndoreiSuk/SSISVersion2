...
class ProgramInfo(tk.Toplevel):
    def __init__(self, master, mode: str, data: dict = None):
        ...
        if self.data is not None: # Editing existing the program
            ...
            if current_college_code is None: # Handle if college code from DB is NULL
                current_college_code = "No Selection"
    ...
    def create_button_callback(self):
        ...
        prog_name = self.name_entry.get().strip() # This is 'programNAME'
        ...
        current_data_payload = {"programID": prog_id_val, "programNAME": prog_name, "collegeCODE": db_college_code}
        if not prog_id_val or not prog_name:
        ...
        if self.mode == 'new':
            ...
            prog_id = self.data.get('programID')

            # Check for the changes
            prog_name = self.data.get("programNAME", "")
            code_college = self.data.get("collegeCODE")
            changed = False
            if prog_name != prog_name  or db_college_code != code_college:
                changed = True

            if not changed:
                return self.dialog("No changes were made.\nPlease try again.", "Information")

            update_payload = {"programID": prog_id, "programNAME": prog_name, "collegeCODE": db_college_code}
            programs.edit(update_payload)
            self.dialog(f"Successfully updated the Program:\n{prog_id.upper()} - {prog_name.upper()}", "Success!!!")
            if hasattr(self.master, 'refresh_program_table'): self.master.refresh_program_table()
            if hasattr(self.master, 'refresh_student_table'): self.master.refresh_student_table()
            self.destroy()