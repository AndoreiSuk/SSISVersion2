...
class StudentInfo(ttk.Toplevel):
    def __init__(self, master, mode: str, data: dict=None):
        ...
        sex_values = ["No Selection", "Male", "Female"]
        ...
        if all_programs_data:
            for program in all_programs_data:
                if program.get("collegeCODE"):
                    program_codes_list.append(program.get("programID"))
        unique_program_codes = list(dict.fromkeys(program_codes_list))
        self.program_option = ttk.Combobox(form_frame, values=unique_program_codes, state='readonly') # programCODE
        ...
        if self.data is not None: # Editing existing student
            ...
            self.program_option.set(program_code_val if program_code_val in unique_program_codes else "No Selection")
    ...
    def create_button_callback(self):
        ...
        if self.mode == 'new':
            if students.check(stud_id_val): return self.dialog("This Student ID already exists!!\nPlease Try Again.", "Error")
            students.insert_one(current_data_payload)
            self.dialog(f"Successfully created the Student #{stud_id_val}", "Success!!!")
            if hasattr(self.master, 'refresh_student_table'): self.master.refresh_student_table()
            self.destroy()
        else:
            original_student_id = self.data.get("studentID") if self.data else None
            
            changed = False
            if self.data:
                if stud_id_val != self.data.get("studentID", "") or \
                   firstname_val != self.data.get("FIRSTNAME", "") or \
                   lastname_val != self.data.get("LASTNAME", "") or \
                   db_sex != self.data.get("SEX") or \
            ...
            if original_student_id != stud_id_val and students.check(stud_id_val):
                return self.dialog("The new Student ID you just entered already exists.", "Update Error")
            students.edit(original_student_id, current_data_payload)
