...
class StudentInfo(ttk.Toplevel):
    def __init__(self, master, mode: str, data: dict=None):
        ...
        program_codes_list_raw = programs.get_program_ids()
        program_codes_list = [str(pcode) for pcode in program_codes_list_raw if pcode]
        if not program_codes_list or program_codes_list[0] != "No Selection":
            if "No Selection" not in program_codes_list:
        ...
        if self.data is not None: # Editing existing student
            self.id_entry.insert(0, str(self.data.get('studentID','')))
            ...
            current_program_from_data = str(self.data.get("programCODE", "")).strip() # Using SQL column name
            selected_program_in_combobox = "No Selection"
            for val_option in program_codes_list:
            ...
            current_year_level_from_data = str(self.data.get("YEAR LEVEL", "")).strip() # Use SQL column name (with space)
    ...
    def create_button_callback(self):
        ...
        if not (len(stud_id_val) == 9 and stud_id_val[4] == '-' and stud_id_val[:4].isdigit() and stud_id_val[5:].isdigit()):
            return self.dialog("Invalid Student ID format.\nExpected: Widodo-NNNN (e.g., 2023-0001)", "Input Error") # Completed the error message
        ...
        if self.mode == 'new':
            ...
            if self.data:
                if firstname_val.upper() != str(self.data.get("FIRSTNAME", "")).upper() or \
                   lastname_val.upper() != str(self.data.get("LASTNAME", "")).upper() or \
                   (db_program_code or "").upper() != (str(self.data.get("programCODE") or "")).upper() or \
                   (db_year_level or "").upper() != (str(self.data.get("YEAR LEVEL") or "")).upper():
                    changed = True
            ...
            edit_payload = {
                ...
                # SEX is not updated from form in edit mode as it's disabled.
                # It's taken from self.data if needed by edit.
