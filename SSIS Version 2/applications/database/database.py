...
def get_db_connection():
    ...
    try:
        ...
        if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
            ...
            print(f"The Database '{DB_CONFIG['database']}' does not exist.")
            try:
...
class Database:
    def __init__(self, table_name: str, pk_col_name: str, columns_sql_names: list, create_table_sql: str):
        ...
        self.initialize()
    def initialize(self):
        # This method calls the SQL table when it doesn't exist using the provided DDL.
    ...
    def execute_query(self, query, params=None, fetch_one=False, fetch_all=False, is_dml=False):
        # This method calls the SQL Query when it doesn't exist using the provided values.
        conn = get_db_connection()
        if conn is None:
    ...
    def get_all(self, keyword=None, filter_criteria=None, sort_by=None, sort_order='ASC'):
        ...
        if self.table_name == 'programs' and keyword:
            ...
            params.append(f"%{keyword}%")
            if 'n/a' in keyword.lower():
                keyword_conditions.append(f"{self.table_name}.collegeCODE IS NULL")
            ...
            if keyword and self.columns_sql_names:
                keyword_conditions = []
                for col_sql_name in self.columns_sql_names:
                    if self.table_name == 'students' and col_sql_name == 'SEX':
                ...
                if self.table_name == 'students':
                    if 'not enrolled' in keyword.lower():
                        keyword_conditions.append("`programCODE` IS NULL")
                    if 'no selection' in keyword.lower():
                        keyword_conditions.append("(`SEX` IS NULL OR `YEAR LEVEL` IS NULL)")
                if keyword_conditions:
                    conditions.append("(" + " OR ".join(keyword_conditions) + ")")
        ...
        if sort_by and sort_by in self.columns_sql_names:
            ...
            orderByClause = f" ORDER BY `{self.pk_col_name}` ASC"
    ...
    def check(self, id_to_check):
        ...
        result = self.execute_query(query, (str(id_to_check),), fetch_one=True)
    ...
    def edit(self, original_pk_val, data: dict):
        if not original_pk_val: return False
        set_clauses = []
        sql_values = []
        for key, value_raw in data.items():
            if key in self.columns_sql_names:
                set_clauses.append(f"`{key}` = %s")
                sql_values.append(value_raw)
        if not set_clauses: return False
        sql_values.append(original_pk_val)
    ...
    def remove(self, id_to_remove):
        ...
        return self.execute_query(query, (str(id_to_remove),), is_dml=True) > 0
    def get_ids(self):
